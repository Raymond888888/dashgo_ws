...
//红绿灯回调函数
void chatterCallback(const std_msgs::Bool _isRed){
	ROS_WARN("*****chatterCallback*****");

	ISRED = _isRed.data;//红绿灯状态赋值
...
int main(int argc, char **argv)
{
	...
	ROS_WARN("*****START");
	 ros::init(argc,argv,"trafficLaneTrack");//初始化ROS节点
        ...
        ros::Subscriber sub = n.subscribe("/isRed", 2, chatterCallback);//订阅红绿灯状态
        ros::Subscriber sub_obs = n.subscribe("/isObs", 2, obsCallback);//订阅障碍物状态
        ros::Subscriber sub_follower = n.subscribe("/turtlebot_follower/cmd_vel_x", 2, followerCallback);//订阅跟踪状态
	...
	while (ros::ok())
	{
		...
		if(ISRED||ISOBS || loseLane >LOSELANE){//有红绿灯或者障碍物或者连续5帧车道线检测失败，停车
